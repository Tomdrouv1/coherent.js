...
- **MongoDB scaffolds work against a real API.** The generated model called `db.collection()`/`db.createCollection()` on the database manager, which exposed neither; the mongo adapter had no write operations at all; the generated init passed a config shape the adapter doesn't read (the URI as the database *name*, plus driver-6-removed `useNewUrlParser`/`useUnifiedTopology` options); and every db scaffold's SIGINT handler called the nonexistent `db.disconnect()`. Fixes: the mongodb adapter and `DatabaseManager` expose `collection(name)` (raw driver collection passthrough, clear error on SQL adapters), `DatabaseManager.close()` falls back to connect-style adapters' `disconnect()`, the generated mongo config/init use `url`/`database`/`options` and bootstrap the unique email index at startup, the generated model normalizes `_id` to a string `id` (converting back via `ObjectId` on lookups), and all scaffold shutdown handlers call `db.close()`. `DatabaseConfig` types gained `url`/`options`; `DatabaseManager` types gained `collection()`/`close()`.
