...
| On Node, `provideContext()` (and `restoreContext()` / `clearAllContexts()` when there is something to remove) throws outside `runWithContext()`: the value used to be attached to the caller's async context, which a keep-alive connection shares with the next request. | Call it inside `runWithContext(() => ...)`, or scope the value with `createContextProvider()` / `runWithContext(fn, { key: value })`. |
| `createContextProvider()` evaluates the function components and function-valued props of its subtree when it is rendered; a context value read later (in an event handler, or by a function the renderer never calls) is no longer visible. | Read the value while rendering and close over it. |
...
| `Model.create(attributes)` applies `fillable` / `guarded` like `fill()` (it used to insert every key, so `create(req.body)` let a client set any column), and always inserts (with a primary key in `attributes` it used to run no query). | List every column callers may set in `fillable`, or set trusted columns with `setAttribute()` before `save()`. |
